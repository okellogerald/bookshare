...
export const config = {
  ...
  flows: [WorkflowFlows.catalog],
  triggers: [{ type: "queue", topic: WorkflowTopics.editionCreated }],
  enqueues: [WorkflowTopics.editionEnriched],
} as const;
...
export async function handler(input: IsbnLookupInput, { enqueue, logger }: any) {
  ...
  try {
    ...
    await enqueue({ topic: WorkflowTopics.editionEnriched, data: enrichedData });
  } catch (error) {
