...
export function WantSubmissionsWorkspace() {
  const { openFlow } = useAdminFlow();
  const [statusFilter, setStatusFilter] = useState("pending");
  const [search, setSearch] = useState("");
  const submissionsQuery = useWantSubmissions(
  ...
  function handleRowClick(submission: WantSubmissionRecord) {
    openFlow({ kind: "review-want-submission", submission });
  }
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Member want requests awaiting review.
              </p>
            </div>
            <Badge
              variant="secondary"
