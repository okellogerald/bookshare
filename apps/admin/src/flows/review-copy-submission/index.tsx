...
import type { CopySubmissionRecord, PgBookWithAuthorsView } from "@/shared/api";
...
function SubmittedData({ submission }: { submission: CopySubmissionRecord }) {
  const fields = [
...
function ReviewForm({
  ...
  const editionResolved = !!selectedEdition || createNewEdition || (createNewBook && createNewEdition !== false);
  // For new books, edition form is always shown inline, so "resolved" means the form exists
  ...
  async function handleApprove() {
    ...
    try {
      let editionId = selectedEdition?.id ?? null;
      ...
      if (createNewBook) {
        ...
        // Always create edition for new book.
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        ...
        {/* Not yet resolved — show search + create */}
        {!bookResolved && (
        ...
        {/* Resolved via selection — compact confirmation */}
        {selectedBook && (
        ...
        {/* Resolved via create — inline form */}
        {createNewBook && (
      ...
      {bookResolved && (
        <section className="space-y-3">
          <StepHeader
            ...
            onClear={editionReady ? clearEdition : undefined}
          />

          {/* Existing book selected — show its editions + option to create */}
          {selectedBook && !selectedEdition && !createNewEdition && (
            <div className="space-y-3 pl-10">
              {editionsQuery.isLoading ? (
                ...
                <>
                  <p className="text-xs text-muted-foreground">
                    {existingEditions.length} edition{existingEditions.length !== 1 ? "s" : ""} found for this book. Select one or create a new one.
                  </p>
                ...
                <p className="text-xs text-muted-foreground">
                  No editions found for this book. Create one below.
                </p>
              )}
          ...
          {/* Edition selected — compact confirmation */}
          {selectedEdition && (
            <div className="ml-10 rounded-lg border border-primary/25 bg-primary/[0.06] px-4 py-3">
          ...
          {/* Creating new edition — inline form */}
      ...
      <div className="flex gap-2 border-t border-border/70 pt-4">
        <Button
          ...
          disabled={processing || !bookResolved || !editionReady}
          className="gap-2"
        >
          {processing && <Loader2 className="h-4 w-4 animate-spin" />}
...
export function ReviewCopySubmissionFlow({
  submission,
  onClose,
}: {
  submission: CopySubmissionRecord;
