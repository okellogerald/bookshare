...
import type { PgMemberProfile, PgBrowseWish } from "@/shared/api";
...
async function fetchCommunityMembers(filters: CommunityFilters): Promise<PgMemberProfile[]> {
...
async function fetchCommunityWishlist(filters: CommunityWishlistFilters): Promise<PgBrowseWish[]> {
  const params = new URLSearchParams();
  params.set("select", "*");
