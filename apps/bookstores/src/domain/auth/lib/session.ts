...
export async function setSession(
  ...
  if (!tokenForApi) {
    throw new Error("Cannot persist a bookstores session without an API token");
  }

  cookieStore.set(BOOKSTORES_SESSION_COOKIE, await encrypt(JSON.stringify(data)), {
    httpOnly: true,
  ...
  cookieStore.set(BOOKSTORES_TOKEN_COOKIE, await encrypt(tokenForApi), {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    sameSite: "lax",
    path: "/",
...
export async function getSession(): Promise<SessionData | null> {
  const cookieStore = await cookies();
  const value = cookieStore.get(BOOKSTORES_SESSION_COOKIE)?.value;
